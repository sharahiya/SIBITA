...
<nav class="bg-white dark:bg-gray-900 fixed w-full z-20 top-0 border-b border-gray-200 dark:border-gray-600">
  <div class="max-w-screen-xl mx-auto flex items-center justify-between p-4">
    ...
    <!-- Desktop Menu -->
    <div class="hidden lg:flex space-x-8 text-sm ml-6">
      ...
      <div class="relative">
        <button id="akunDropdownButton" type="button"
          class="flex items-center space-x-1 text-gray-900 dark:text-white hover:text-blue-700
        ...
        <div id="akunDropdown"
          class="hidden absolute mt-2 w-40 right-0 bg-white dark:bg-gray-700 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 dark:divide-gray-600 z-50">
          <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
            <li><a href="{{ route('manajemenakun') }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600">Kelola Akun</a></li>
            <li><a href="{{ route('daftarakunadmin') }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600">Daftar Akun</a></li>
          </ul>
        </div>
      </div>
    ...
    <div class="flex items-center space-x-4">
      ...
      <a href="{{ route('notifikasiadmin') }}" class="relative">
        <svg class="w-6 h-6 text-gray-800 dark:text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round"
            d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14V11a6 6 0 00-12 0v3c0 .386-.146.75-.405 1.045L4 17h5m6 0a3 3 0 11-6 0">
          </path>
        </svg>
        <span
          class="absolute top-0 right-0 inline-block w-4 h-4 text-xs text-white bg-red-600 rounded-full text-center">3</span>
      </a>
      ...
      <div class="relative">
        ...
        <div id="user-dropdown"
          class="hidden absolute right-0 mt-2 w-48 bg-white divide-y divide-gray-100 rounded-lg shadow-lg dark:bg-gray-700 dark:divide-gray-600">
          <div class="px-4 py-3">
          ...
          <ul class="py-2">
            <li><a href="{{ route('resetpass') }}"
                class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-white dark:hover:bg-gray-600">Reset
                Password</a></li>
            <li><a href="#"
                class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-white dark:hover:bg-gray-600">Sign
                out</a></li>
          </ul>
        </div>
      </div>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener("click", function (e) {
      if (!akunDropdown.contains(e.target) && !akunDropdownButton.contains(e.target)) {
    ...
    document.addEventListener("click", function (event) {
      ...
      if (!akunDropdown.contains(event.target) && !akunDropdownButton.contains(event.target)) {
        akunDropdown.classList.add("hidden");
      }
